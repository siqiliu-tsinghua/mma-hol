...
HOLTest`runTests["terms: mkAbs canonicalization", Module[{alpha, beta, x, y, idα, t2, expected2},
  ...
  idα = mkAbs[x, x];
  HOLTest`assertEq[
    idα, abs[bvar[0, alpha], bvar[0, alpha], "x"],
    "λx:α.x canonicalizes both slots"
  ];
  HOLTest`assertEq[
    destAbs[idα], {bvar[0, alpha], bvar[0, alpha], "x"},
  ...
  expected2 = abs[
    bvar[0, alpha],
    abs[bvar[0, beta], bvar[1, alpha], "y"],
    "x"
  ];
  HOLTest`assertEq[t2, expected2, "λx.λy.x uses bvar[1] for outer reference"];
  HOLTest`assertThrows[
    mkAbs[boolTy, x],
...
HOLTest`runTests["terms: vsubst capture-avoidance", Module[{alpha, x, y, lamYx, subbed},
  ...
  HOLTest`assertEq[
    lamYx,
    abs[bvar[0, alpha], x, "y"],
    "λy.x canonical form (x free inside)"
  ];
  subbed = vsubst[<|x -> y|>, lamYx];
  HOLTest`assertEq[
    subbed,
    abs[bvar[0, alpha], y, "y"],
    "substituted (binder is bvar[0], origin 'y' is only a label)"
  ];
  HOLTest`assertEq[
    freesIn[subbed], {y},
...
HOLTest`runTests["terms: vsubst basics", Module[{alpha, x, y, f, app, r},
  ...
  HOLTest`assertEq[
    vsubst[<|var["_b0", alpha] -> y|>, mkVar["_b0", alpha]],
    y,
    "_b0-named free var is just a var; substitutes normally"
  ];
]];
...
HOLTest`runTests["terms: instType", Module[{alpha, beta, x, lam, theta, result},
  ...
  HOLTest`assertEq[
    result,
    abs[bvar[0, boolTy], bvar[0, boolTy], "x"],
