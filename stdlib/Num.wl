...
multNonzeroThm::usage = "multNonzeroThm — ⊢ ∀m n. ¬ (m = 0) ⇒ ¬ (n = 0) ⇒ ¬ (m * n = 0). No zero divisors in ℕ.";
...
(* addRightCommThm : ⊢ ∀a b c. (a + b) + c = (a + c) + b *)
(* General add fact placed here for proximity to dividesShiftThm, its    *)
(* only current user. (a+b)+c = a+(b+c) = a+(c+b) = (a+c)+b.             *)

addRightCommThm =
  Module[{aV, bV, cV, assoc1, commBC, apComm, assoc2, chain},
    aV = mkVar["a", numTy];
    bV = mkVar["b", numTy];
    cV = mkVar["c", numTy];

    assoc1 = HOL`Bool`SPEC[cV, HOL`Bool`SPEC[bV, HOL`Bool`SPEC[aV, addAssocThm]]];
    (* ⊢ (a + b) + c = a + (b + c) *)
    commBC = HOL`Bool`SPEC[cV, HOL`Bool`SPEC[bV, addCommThm]];
    (* ⊢ b + c = c + b *)
    apComm = HOL`Equal`APTERM[mkComb[plusConst[], aV], commBC];
    (* ⊢ a + (b + c) = a + (c + b) *)
    assoc2 = HOL`Bool`SPEC[bV, HOL`Bool`SPEC[cV, HOL`Bool`SPEC[aV, addAssocThm]]];
    (* ⊢ (a + c) + b = a + (c + b) *)
    chain = TRANS[TRANS[assoc1, apComm], HOL`Equal`SYM[assoc2]];
    (* ⊢ (a + b) + c = (a + c) + b *)
    HOL`Bool`GEN[aV, HOL`Bool`GEN[bV, HOL`Bool`GEN[cV, chain]]]
  ];

(* dividesShiftThm :                                                      *)
(*   ⊢ ∀d x t δ. divides d δ ⇒                                            *)
(*       (divides d ((x + δ) + t) = divides d (x + t))                    *)
(*                                                                        *)
(* Cooper atom-level periodicity. Rearrange (x+δ)+t to (x+t)+δ via        *)
(* addRightComm, then dividesAddEqThm at y = δ collapses the +δ.           *)

dividesShiftThm =
  Module[{dV, xV, tV, deltaV, dDeltaTm, rearr, apDiv, addEqInst,
          ddHyp, addEqMP, chain, disch},
    dV = mkVar["d", numTy];
    xV = mkVar["x", numTy];
    tV = mkVar["t", numTy];
    deltaV = mkVar["delta", numTy];

    dDeltaTm = dividesTm[dV, deltaV];

    rearr = HOL`Bool`SPEC[tV,
      HOL`Bool`SPEC[deltaV, HOL`Bool`SPEC[xV, addRightCommThm]]];
    (* ⊢ (x + δ) + t = (x + t) + δ *)
    apDiv = HOL`Equal`APTERM[mkComb[dividesConst[], dV], rearr];
    (* ⊢ divides d ((x+δ)+t) = divides d ((x+t)+δ) *)
    addEqInst = HOL`Bool`SPEC[deltaV,
      HOL`Bool`SPEC[plusTm[xV, tV], HOL`Bool`SPEC[dV, dividesAddEqThm]]];
    (* ⊢ divides d δ ⇒ (divides d ((x+t)+δ) = divides d (x+t)) *)
    ddHyp = ASSUME[dDeltaTm];
    addEqMP = HOL`Bool`MP[addEqInst, ddHyp];
    (* {d|δ} ⊢ divides d ((x+t)+δ) = divides d (x+t) *)
    chain = TRANS[apDiv, addEqMP];
    (* {d|δ} ⊢ divides d ((x+δ)+t) = divides d (x+t) *)
    disch = HOL`Bool`DISCH[dDeltaTm, chain];
...
(* ============================================================ *)
(* general ℕ / order / divides arithmetic lemmas                 *)
(* (gcd / prime / Euclid's lemma migrated to FTA.wl 2026-06)     *)
(* ============================================================ *)
...
(* ============================================================ *)
(* exDiv — exact quotient via Hilbert ε (migrated from Rat.wl). *)
(* exDiv n g = ε c. n = g * c. When g divides n this is the     *)
(* unique quotient, and exDivThm falls straight out of selectAx *)
(* with no DIV/MOD-uniqueness lemma. + multNonzero (no ℕ zero   *)
(* divisors). Used downstream by FTA (coprimeReduced/exDivSelf) *)
(* and Int (intDivNat).                                         *)
(* ============================================================ *)

zeroN[]   := zeroConst[];
oneN[]    := mkComb[sucConst[], zeroConst[]];
