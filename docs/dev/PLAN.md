...
#### 连续函数 demo —— `demos/Continuous.wl` + `demos/continuous.nb`（✅ DONE，含 EVT）
...
- ✅ **EVT `continuousEVTThm`**：`⊢ ∀f a b. continuous f ⇒ a≤b ⇒ ∃cMax. ci a b cMax ∧ ∀x. ci a b x ⇒ f x ≤ f cMax`（0 hyp）。实现：T=IMAGE f [a,b] 紧⇒闭+有界+非空（f a∈T）；`M=realSup T`（`realSupUpperThm`/`realSupLeastThm`）；**M∈T** 序列法——`evtSeq=λn. @z. T z ∧ M−invSuccRadius n<z`，逐 n 存在性=`EXCLUDEDMIDDLE`+反证（`realSupLeastThm` 得 M≤M−r，`realLeLtTransThm`+`realLtIrreflThm` 收 F），`invSuccRadiusTendstoZeroThm` 挤压（`realLtTransThm`）得 `tendsto evtSeq M`，`limitMemOfClosedThm`⇒`T M`；`ctImageAt` 拆 `∃cMax. ci cMax ∧ M=f cMax`，`realSupUpperThm` 得 f x≤M=f cMax。**线性 ℝ 算术全部 hoisted 成 5 条 `realArithProve` 引理**（`leSub/dropZero/dNN/dLtR/negEq`）再 `ctSpecAll`——避免 `realArithProve[goal], {args}` 的 `]` 漏配。**坑**：`leTm=concl[specHAll][[1,2]]`（蕴含的前件是 `[[1,2]]` 不是 `[[1]]`，后者是 `⇒A` 偏应用→ASSUME 非 bool throw）；`numTy`/`&ℝ0` 从 `invSuccRadiusPosThm`/`invSuccRadiusTendstoZeroThm` 的 concl 提取（Num private 不可裸用）。
- ✅ **笔记本**（生成器 `build_continuous_nb.wls`）：`demos/continuous.nb`，含 EVT 段，全 cell 验证。
- 验收：恢复 bootstrap.mx + `Get demos/Continuous.wl` 的单独脚本迭代；demos/ 不入正式套件。demo 全部 7 条公开定理 isThm=True、0 hyp。
