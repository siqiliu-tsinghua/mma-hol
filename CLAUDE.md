...
**M7 stdlib** — Pair, Sum, Option, Set done. **Num done through M7-3** (Peano via `ind`+INFINITY_AX; iteration theorem; `+ × ^`; `≤ <`; strong induction; well-ordering; division `m = n*q+r`; `DIV`/`MOD`; `divides`; `gcd` + universal property; `prime`; Euclid's lemma — all no integer Bezout). FTA stage 1 (`primeDivExistsThm`) landed in `stdlib/FTA.wl` after M7-4 — stages 2 (factorization existence) and 3 (uniqueness) still queued. **List done through M7-4-d** (α list as `num → α option` finite-support subtype: NIL/CONS, injectivity, NIL≠CONS disjointness, list induction; `LENGTH`/`HD`/`TL`; list iteration theorem; `FOLDR`/`FOLDL`/`APPEND`/`MAP`/`FILTER` + NIL/CONS clauses). **`COND` + COND_CLAUSES** added to Bool.wl (needed by FILTER).
