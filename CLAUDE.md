...
**CARD/∑ — through ITSET + FINITE_RECURSION (M7-4-f.4)** — FINREC machinery (f.1–f.3) + acrossN uniqueness (f.4.a) + **ITSET** (`= λf s b. @z. ∃n. FINREC f b n s z`) with clauses `itsetEmptyThm` (unconditional) and `itsetInsertThm` (`comm ⇒ FINITE s ⇒ ITSET f (x INSERT s) b = COND (x∈s) (ITSET f s b) (f x (ITSET f s b))`, the FINITE_RECURSION clause in HOL Light's COND form). Supporting set identities + `inMemAbsorbThm`/`notInMemDelInsertThm`. Local helpers: `extractItset` (ε-extraction via selectAx + acrossN uniqueness), `eqfIntro` (p=F from ¬p via DEDUCTANTISYM). Remaining: f.5 CARD, f.6 ∑.

**Next**: M7-4-f.5 (`CARD = λs. ITSET (λx n. SUC n) s 0` — trivial comm step `λx n. SUC n` satisfies ∀x y a. SUC(SUC a) = SUC(SUC a); derive CARD ∅ = 0 and CARD (x INSERT s) = if x∈s then CARD s else SUC(CARD s)) → f.6 ∑; then M7-δ ARITH, M7-5 Int, … per PLAN. (List `MEM`/`EL`/`REVERSE`/`ALL`/`EX` etc. on demand.) **Audit issues parked in `TODO.md`.**
