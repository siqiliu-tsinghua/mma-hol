...
**CARD/∑ started (M7-4-f)** — `FINREC` count-indexed fold graph (f.1) + exchange-lemma helpers/base (f.2.a). Done: `finrecZero/Suc(App)Thm`, `deleteCommThm` (`(s\x)\y = (s\y)\x`), `sDelEmptyImpEqThm` (`s\x = ∅ ∧ y∈s ⇒ y = x`), `finrecExchangeBaseThm` (n=1 case via inversion + Leibniz). Remaining: f.2.b exchange induction step (x=y / x≠y case-split, IH on s\y, comm-swap), f.3 uniqueness/existence, f.4 ITSET + FINITE_RECURSION, f.5 CARD, f.6 ∑. See memory `m7_finite_and_number_tower_plan` for the detailed route.

**Next**: continue M7-4-f.2.b (exchange induction step) → … → CARD/∑; then M7-δ ARITH, M7-5 Int, … per PLAN. (List `MEM`/`EL`/`REVERSE`/`ALL`/`EX` etc. on demand.) **Audit issues parked in `TODO.md`.**
