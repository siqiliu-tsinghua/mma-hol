...
**Finite.wl foundation done (M7-4-e)** — `INSERT`/`SING`/`DELETE` added to Set.wl; **inductive `FINITE`** (smallest predicate with ∅ + INSERT-closure) with `finiteEmptyThm`/`finiteInsertThm`/`finiteSingThm`, strong `finiteInductThm` (FINITE s in the step), `finiteUnionThm`, `finiteSubsetThm`, `finiteDeleteThm`. `finiteSubsetThm`'s INSERT step does `x∈t` case-split + conditional set identities via kernel `SUBS` (membership Leibniz) — no MESON; membership eqs built by INST-ing Set.wl theorems (not simpConv, whose basicSimpset absorbs `a∨(b∧¬a)`). Pending: `finiteImageThm` (needs ∃) + `CARD`/`∑`.

**Next**: `finiteImageThm` (∃, via CHOOSE/EXISTS or MESON) then `CARD`/`∑` (needs order-independent set-fold); then M7-δ ARITH, M7-5 Int, … per PLAN. (List `MEM`/`EL`/`REVERSE`/`ALL`/`EX` etc. on demand.) **Audit issues parked in `TODO.md`.**
...
- `stdlib/Finite.wl`: inductive `FINITE` (`finiteConst`/`finiteDefThm`); `finiteEmptyThm`/`finiteInsertThm`/`finiteSingThm`; strong `finiteInductThm`; `finiteUnionThm`/`finiteSubsetThm`/`finiteDeleteThm`. (IMAGE closure + CARD/∑ pending.) Local helpers: `propSetEq` (propositional set-eq via simpConv+propTaut+funcExt), `setExtFromInEq`, `condDeleteEqLeft`/`Right`.
