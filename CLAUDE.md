...
**M7-δ session 1 done (auto/Arith.wl)** — Cooper QE scaffolding only: linTerm AST (`linTerm[const, <|name→coef|>]` with smart constructors `linZero`/`linConst`/`linVar`/`linAdd`/`linScale`/`linNeg`/`linSub`), HOL ↔ linTerm conversion (`parseLin` recursive descent over 0/SUC/+/`*-by-literal`; `buildLin` emits the canonical right-assoc `c_0 + c_1*x_1 + … + c_k*x_k`). `arithProve` and `ARITH` are stubs that throw `arith-stub`. Atom/Formula AST, NNF, and the Cooper QE engine itself are queued for next sessions. Loaded last in build_snapshot/run_all.

**Next**: M7-δ session 2 — add aAtom (=, ≤, divides + their negations after NNF) + aForm tree + parser/builder + NNF at AST level, then start wiring the Cooper QE skeleton (per-step HOL certificate for NNF first, then the ∃-elimination steps). Capstone TBD (PLAN's nominal example `∀m n. m + n ≤ m*n + 2` is in fact false at m=0,n≥3; a real Presburger-only capstone like `∀m n. m ≤ m + n` or `∀m n. m + n + 1 ≤ SUC (m + n) + 1` is more accurate). Alternative: M7-5 `stdlib/Int.wl`. **Audit issues parked in `TODO.md`.**
