...
M1 + M2 + M3 + M4 + M5 + M6a + M6c done. Next is M7 (stdlib + 5 automation tactics); M6b (notebook MakeBoxes) remains deferrable.
...
`tests/bootstrap_tests.wl` has the M3 capstone ⊢ T. `tests/bool_tests.wl` has the M4 capstone — `⊢ ∀x y. x = y ⇒ y = x`. `tests/tactics_tests.wl` has the M5 acceptance — tactic-style proofs of `⊢ T`, `⊢ ∀p. p ⇒ p`, `⊢ ∀p q. p ∧ q ⇒ q ∧ p`, `⊢ ∀p q. p ⇒ q ⇒ p ∧ q`, plus a step-by-step goalstack round-trip on the same theorems. `tests/printer_tests.wl` covers atoms / app / each registered operator / nested precedence / binder chains / α-renaming collisions / ASCII vs Unicode mode / formatThm / `tDef` capstone (`⊢ T = (λx. x) = λx. x`). `tests/parser_tests.wl` covers types (atoms / arrow / errors), term atoms / applications / equality / `∧ ∨ ⇒ ¬` / ASCII aliases / `λ` and `∀ ∃` binders / type ascription / errors / type unification failures, plus a 15-term parse → print → parse round-trip and the `tDef` capstone via the parser. Running `wolframscript -file tests/run_all.wls` at repo root passes 519/0 in ~4s. Next: M7 — stdlib (Pair/Sum/Option/Set/Num/Int/Rat/Real/List/Finite) + 5 automation tactics (`MESON`/`SIMP`/`SET`/`ARITH`/`REAL_ARITH`). M6b (notebook MakeBoxes + goal viz) is deferrable to any spare slot.
