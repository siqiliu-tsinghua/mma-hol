...
**M7 automation** — `MESON` (auto/Meson.wl) and `SIMP`/`asmSimp` (auto/Simp.wl) substantially complete; `SET` (auto/Set.wl) done. `ARITH` (auto/Arith.wl) **capstone landed** — ∃-SAT prover works (S9-11); oracle+verifier Farkas path proves ∀ goals (capstone `∀m n p. m≤n ⇒ m+p≤n+p` in one `ARITH[]`); remaining = non-unit-multiplier generalization (see below). Pending: MESON α-5-c Brand modulation + NOT_FORALL/NOT_EXISTS NNF schemas (deferred till a goal forces them); `REAL_ARITH` (M7-ε) not started.
...
**M7-δ Farkas engine DONE (capstone scope, auto/Arith.wl).** `linNormConv[t]` → `⊢ t = buildLin[parseLin[t]]` (proof-producing linear normalizer: caAdd flattens via addAssoc + folds monomials via caInsert; sucEqAddOne reduces SUC-of-sum; covers sort + const-fold + SUC). `arithProveForall`: peel ∀ (origin names reused → GEN reconstructs goal exactly) + ⇒, ASSUME hyps, negate concl (notLeqEqLt/notLtEqLeq), `farkasRefute` → F, CCONTR/DISCH/GEN. `farkasRefute`: untrusted `farkasUnitCert` (λ∈{0,1} subset search, smallest set whose Σ(Lᵢ-Rᵢ) has zero var part + positive const) → verifier sums (leqAddMono), normalizes (linNormConv), commutes const right, cancels shared var part (leqAddLeftCancel), ground-contradicts (notLeqEqLt + proveGroundLt → NOTELIM). `arithProve` dispatcher gains the ∀x:num branch.

**Next**: M7-δ non-unit generalization — (a) `linNormConv` coefficient-merge (same var both operands; throws `arith-norm-merge` now) + literal scaling `k·expr`; (b) general WL `LinearProgramming` multipliers (λ>1) wired through `leqMultLeftThm` scaling in `farkasRefute`; (c) implicit `0 ≤ xᵢ` facts so ℕ-only truths like `∀x. x ≤ x+x` close. Each unblocks the next. Then M7-5 `stdlib/Int.wl`. **Audit issues parked in `TODO.md`.**
