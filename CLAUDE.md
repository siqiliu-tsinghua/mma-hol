...
- `Bool.wl` (M4b): `TRUTH`, `EQTINTRO`/`EQTELIM`, `GEN`/`SPEC`/`ISPEC`, `CONJ`/`CONJUNCT1`/`CONJUNCT2`, `MP`/`DISCH`/`UNDISCH`, `NOTINTRO`/`NOTELIM`/`CONTR`, `EXISTS`/`CHOOSE`, `DISJ1`/`DISJ2`/`DISJCASES`, `EXCLUDEDMIDDLE`/`CCONTR`, `freshName`.
...
- `stdlib/List.wl`: `α list` = `num → α option` finite-support subtype (`isListP` carrier predicate). `NIL`/`CONS`, `repNil`/`repConsHead`/`repConsTail`, `consInjThm`, `nilNotEqConsThm`, `listInductionThm`; helpers `optionCasesThm`, tail/shift toolkit, `funcExtThm`. (M7-4-a foundation; ops M7-4-b+.)
