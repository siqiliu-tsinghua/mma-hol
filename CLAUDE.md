...
**Next**: ARITH residual (narrow, do when a goal forces it) — (a) `linNormConv` full `k·(a+b)` distribution + right-literal `var·klit` (currently throw `arith-norm`); (b) disjunction/DNF splitting (reuse the Cooper-era `nnfForm`/`nnfConv`) for non-conjunctive goals. ARITH's real consumers are M8 (analysis index/interval arithmetic); the current stdlib is almost entirely structural equalities, so there are no drop-in ARITH targets there yet. Then M7-5 `stdlib/Int.wl`. **Audit issues parked in `TODO.md`.**
