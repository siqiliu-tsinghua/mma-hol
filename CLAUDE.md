...
**CARD/∑ started (M7-4-f)** — FINREC machinery + across-n uniqueness: f.1 `FINREC` graph + clauses, f.2 `finrecExchangeThm`, f.3 `finrecUniqueThm` + `finrecExistsThm`, **f.4.a `finrecAcrossNUniqueThm`** (`⊢ comm ⇒ ∀n m s a1 a2. FINREC f b n s a1 ∧ FINREC f b m s a2 ⇒ a1 = a2` — across-count uniqueness, the keystone for ITSET via Hilbert ε). Supporting set identities: `deleteCommThm`, `sDelEmptyImpEqThm`, `inMemAbsorbThm`, `notInMemDelInsertThm`. Remaining: f.4.b ITSET def + clauses + FINITE_RECURSION, f.5 CARD, f.6 ∑.

**Next**: M7-4-f.4.b (`ITSET f s b = @z. ∃n. FINREC f b n s z`; derive `ITSET f ∅ b = b` and the FINITE_RECURSION clause `FINITE s ⇒ ITSET f (x INSERT s) b = if x∈s then ITSET f s b else f x (ITSET f s b)`) → f.5/f.6 CARD/∑; then M7-δ ARITH, M7-5 Int, … per PLAN. (List `MEM`/`EL`/`REVERSE`/`ALL`/`EX` etc. on demand.) **Audit issues parked in `TODO.md`.**
