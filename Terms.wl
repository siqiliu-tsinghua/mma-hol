...
var::usage     = "var[name, type] — free-variable head. Construct via mkVar.";
...
bvar::usage    = "bvar[k_Integer, type] — canonical bound-variable head. Index k counts binders outward (0 = innermost). Produced only by mkAbs / instType.";

mkVar::usage      = "mkVar[name, ty] returns var[name, ty] after validating the name is a nonempty string.";
mkComb::usage     = "mkComb[f, x] returns comb[f, x] if typeOf[f] is fun[dom, rng] and typeOf[x] === dom.";
mkAbs::usage      = "mkAbs[v, body] returns an α-canonical abs. v must be a var; occurrences of v in body are replaced by bvar[depth, ty] for their depth.";
...
freesIn::usage    = "freesIn[term] returns the sorted list of distinct free (named) variables.";
vsubst::usage     = "vsubst[theta, term] substitutes terms for free variables; theta is keyed by var. Requires type-preserving entries; capture-free by canonical form.";
...
mkAbs[v : var[n_String, _], body_] :=
  Module[{ty = v[[2]], walked},
    walked = bindWalk[body, v, 0];
    abs[bvar[0, ty], walked, n]
  ];
mkAbs[v_, _] :=
  HOL`Error`holError["term", "mkAbs: first arg must be a var", <|"got" -> v|>];

bindWalk[v : var[_String, _], target_, depth_Integer] :=
  If[v === target, bvar[depth, v[[2]]], v];
bindWalk[bv : bvar[_Integer, _], _, _] := bv;
...
freesIn[t_] := Sort[DeleteDuplicates[freesWalk[t]]];

freesWalk[v : var[_, _]] := {v};
freesWalk[bvar[_, _]] := {};
freesWalk[const[_, _]] := {};
freesWalk[comb[f_, x_]] := Join[freesWalk[f], freesWalk[x]];
freesWalk[abs[_, body_, _]] := freesWalk[body];
...
validateVsubst[theta_Association] :=
  KeyValueMap[
    Function[{k, v},
      If[! MatchQ[k, var[_String, _]],
        HOL`Error`holError["term", "vsubst: key must be a var",
          <|"key" -> k|>]];
      If[k[[2]] =!= typeOf[v],
...
instTypeImpl[theta_, bvar[k_Integer, t_]] := bvar[k, typeSubst[theta, t]];
